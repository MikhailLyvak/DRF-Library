    ...
    def payment_success(self, request):
        try:
            ...
            if session.payment_status == "paid":
                ...
                payment.save()
                return Response("Payment marked as PAID", status=status.HTTP_200_OK)
            else:
                return Response("Payment not paid", status=status.HTTP_400_BAD_REQUEST)
        except (KeyError, Payment.DoesNotExist, SignatureVerificationError):
            raise Http404
